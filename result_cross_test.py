...
class PLModel(pl.LightningModule):
    ...
    def _shared_step(self, batch, mode, batch_idx=None, prefix="train"):
        ...
        if self.training and self.fds:
            ...
            loss = mse_loss(input=y_hat.view(-1), target=y)
            ...
            y_hat = y_hat.squeeze()
            loss = mse_loss(input=y_hat, target=y)
            metric = mse_loss(input=y_hat, target=y)
...
def main():
    ...
    for i in range(32):
        ...
        trainx = np.load('/home/mininet/cha/TransRR/capnobase_cross_domain/trainx_{}.npy'.format(i))[:, :, 0:2]
        trainy = np.load('/home/mininet/cha/TransRR/capnobase_cross_domain/trainx_{}.npy'.format(i))[:, :, -1]
        validx = np.load('/home/mininet/cha/TransRR/capnobase_cross_domain/validx_{}.npy'.format(i))[:, :, 0:2]
        validy = np.load('/home/mininet/cha/TransRR/capnobase_cross_domain/validx_{}.npy'.format(i))[:, :, -1]
        testx = np.load('/home/mininet/cha/TransRR/capnobase_cross_domain/testx_{}.npy'.format(i))[:, :, 0:2]
        testy = np.load('/home/mininet/cha/TransRR/capnobase_cross_domain/testx_{}.npy'.format(i))[:, :, -1]
        ...
        if args1.is_train == 'yes':  # 训练
            ...
            loss_callback = LossLogger(save_dir='./result', time=new_name)
            ...
            for i_i in range(52):
                ...
                model = PLModel.load_from_checkpoint('./checkpoint/{}___best_model.ckpt'.format(i_i))
                model.eval()
                ...
                # 保存模型预测结果
                with open('result/test_predictions_BC{}_{}.pkl'.format(i,i_i), 'wb') as f:
                    pickle.dump(test_predictions, f)
                with open('result/test_reals_BC{}_{}.pkl'.format(i,i_i), 'wb') as f:
    ...
    for i in range(52):
        ...
        # 定义数据集
        print("#########################第{}次训练####################".format(i))
        trainx = np.load('/home/mininet/cha/TransRR/bidmc_cross_domain/trainx_{}.npy'.format(i))[:, :, 0:2]
        trainy = np.load('/home/mininet/cha/TransRR/bidmc_cross_domain/trainx_{}.npy'.format(i))[:, :, -1]
        validx = np.load('/home/mininet/cha/TransRR/bidmc_cross_domain/validx_{}.npy'.format(i))[:, :, 0:2]
        validy = np.load('/home/mininet/cha/TransRR/bidmc_cross_domain/validx_{}.npy'.format(i))[:, :, -1]
        testx = np.load('/home/mininet/cha/TransRR/bidmc_cross_domain/testx_{}.npy'.format(i))[:, :, 0:2]
        testy = np.load('/home/mininet/cha/TransRR/bidmc_cross_domain/testx_{}.npy'.format(i))[:, :, -1]
        ...
        if args1.is_train == 'yes':  # 训练
            ...
            for i_i in range(32):
                ...
                model = PLModel.load_from_checkpoint('./checkpoint/{}___best_model-v1.ckpt'.format(i_i))
                model.eval()
                ...
                # 保存模型预测结果
                with open('result/test_predictions_CB{}_{}.pkl'.format(i,i_i), 'wb') as f:
                    pickle.dump(test_predictions, f)
                with open('result/test_reals_CB{}_{}.pkl'.format(i,i_i), 'wb') as f:
                    pickle.dump(test_reals, f)
